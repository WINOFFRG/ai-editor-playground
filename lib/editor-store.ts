...
// Extended schema with lists
export const schema = new Schema({
  nodes: addListNodes(basicSchema.spec.nodes, "paragraph block*", "block"),
  marks: basicSchema.spec.marks,
});

// Create slash command plugin for the new React ProseMirror API
      ...
      handleKeyDown: (view, event) => {
        ...
        if (event.key === "/") {
          ...
          if (isAtStartOfLine || isAfterWhitespace) {
            // Call the callback to open AI prompt
            onSlashCommand?.();

            return true; // Prevent default behavior
          }
        }
        // Return false for all other keys to allow normal behavior
        return false;
      },
    },
  });
...
// Helper function to create editor state
